      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            id="btn-add-slot"
            onClick={openCreate}
            className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 rounded-xl font-semibold transition shadow-lg shadow-indigo-500/20"
          >
        ...
        {/* Active class chip */}
        {activeSlot && (
          <div className="glass-panel flex items-center gap-3 px-5 py-3 border border-emerald-500/30 bg-emerald-500/5 rounded-xl text-emerald-300 text-sm">
        ...
        {/* Error */}
        {error && (
          <div className="glass-panel border border-red-500/30 bg-red-500/5 text-red-400 px-5 py-3 rounded-xl text-sm">
            {error}
          </div>
        )}
            ...
            <form onSubmit={saveSlot} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="sm:col-span-2">
                ...
                <input required value={form.subject_name}
                  ...
                  className="w-full px-4 py-2.5 bg-slate-800/60 border border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/40 text-white placeholder-slate-500 transition"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Day</label>
                <select value={form.day_of_week} onChange={(e) => setForm({ ...form, day_of_week: e.target.value })}
                  className="w-full px-4 py-2.5 bg-slate-800/60 border border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/40 text-white transition">
              ...
              <div className="sm:col-span-2 flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setShowForm(false)}
                  className="px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium transition">Cancel</button>
                <button id="btn-save-slot" type="submit" disabled={saving}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition disabled:opacity-60">
        ...
        {deleteId !== null && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="glass-panel border border-slate-700 rounded-2xl p-8 max-w-sm w-full shadow-2xl">
              ...
              <h3 className="text-xl font-bold text-center text-white mb-2">Delete Slot?</h3>
              <p className="text-slate-400 text-sm text-center mb-6">
                Attendance records will be preserved but unlinked from this subject.
              </p>
              <div className="flex gap-3">
                <button onClick={() => setDeleteId(null)}
                  className="flex-1 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium transition">Cancel</button>
                <button id="btn-confirm-delete" onClick={confirmDelete}
                  className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-semibold transition">Delete</button>
              </div>
            </div>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="glass-panel rounded-2xl p-16 text-center">
            ...
            <p className="text-slate-400 font-medium">No timetable slots yet.</p>
                  ...
                  return (
                    <tr key={slot.id}
                      className={`group transition-colors ${
                        isNow ? "bg-emerald-500/5 hover:bg-emerald-500/10" : "hover:bg-slate-800/30"
                      }`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          {isNow && <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />}
                          <span className="font-semibold text-white">{slot.subject_name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-slate-300">{slot.day_of_week}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => openEdit(slot)}
                            className="p-2 rounded-lg bg-slate-800 hover:bg-indigo-500/20 hover:text-indigo-400 text-slate-400 transition" title="Edit">
                            <Pencil className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeleteId(slot.id)}
                            className="p-2 rounded-lg bg-slate-800 hover:bg-red-500/20 hover:text-red-400 text-slate-400 transition" title="Delete">
